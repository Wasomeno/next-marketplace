...
export default async function EditOrderPage({ params }: StoreOrderPageProps) {
    ...
    <div className="w-full space-y-6">
      <div className="space-y-2">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="w-fit rounded-md bg-gray-200 px-2.5 py-2 text-xs font-medium lg:px-4 lg:text-sm">
            {order?.status}
          </div>
        </div>
        <div>
      ...
      <div className="flex justify-end py-2">
        <OrderChangeStatusButton status={order?.status as OrderStatus} />
      </div>
    </div>
  )
}
